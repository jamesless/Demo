...
ID	VARCHAR(32)	  primary key 	comment '随机32位ID，表示一个循环依赖闭环ID' ,
DEPENDENCE_ID	VARCHAR(400)	comment '单项依赖关系ID',
TYPE	CHAR(1)	comment '0：WAR DEPENDENCE_ID数据来自 T_DEPEN_ANALYSIS 1：JAR DEPENDENCE_ID数据来自 T_APP_DEPENDENCES',
STATUS	CHAR(1)	 COMMENT '0: 使用中 1: 未使用',
CREATE_TIME	TIMESTAMP,
UPDATE_TIME	TIMESTAMP	
);
